...
def test_wide_mode_internal(trace_type, x, y, color, orientation):
    ...
    print(type(df_out.sort_index(axis=1)))
    print(type(pd.DataFrame(expected).sort_index(axis=1)))
    assert_frame_equal(
...
def test_wide_mode_internal_special_cases(df_in, args_in, args_expect, df_expect):
    ...
    # print(type(df_out.sort_index(axis=1)))
    # print(type(df_expect.sort_index(axis=1)))
    print(df_out.sort_index(axis=1))
    print(df_expect.sort_index(axis=1)._to_pandas())
    assert_frame_equal(
        df_out.sort_index(axis=1),
